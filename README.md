...
- [PowerPoint presentation](presentation/Support_Ticket_Router_Presentation.pptx), with editable charts and speaker notes.
- [Video narration script](presentation/VIDEO_SCRIPT.md), matching the slides.
- [Full comparison report](results/REPORT.md) and [results notebook](Comparison_Results.ipynb).
- [Step-by-step walkthrough](PROJECT_WALKTHROUGH.md) and [submission guide](SUBMISSION_GUIDE.md).
- [Original project attribution](ATTRIBUTION.md).

Repository: https://github.com/nishokvg/finetuning-compare

This checkout includes the code, prepared dataset and saved evidence. Downloaded model weights and Python environments are excluded from GitHub. On the original Mac, ignored local symlinks reuse those large files from the earlier project directory. A fresh clone needs the setup and model-download steps below. The recorded demo uses the local merged export, not a permanently running cloud endpoint.
